...
namespace InvoiceRecordExportTool.Task
{
    public class Import
    {
        ...
        /// <returns></returns>
        public DataTable ImportExcelToDt(string fileAddress,int typeid)
        {
            var dt = new DataTable();
            try
            {
                //使用NPOI技术进行导入EXCEL至DATATABLE
                var importExcelDt = OpenExcelToDataTable(typeid,fileAddress);
                //将从EXCEL过来的记录集为空的行清除
        ...
        private DataTable OpenExcelToDataTable(int typeid,string fileAddress)
        {
            ...
            //获取动态生成绑定字段临时表
            dt = typeid == 0 ? tempDtList.MakeCustomerBasicTemp() : tempDtList.MakeMaterialBasicTemp();
